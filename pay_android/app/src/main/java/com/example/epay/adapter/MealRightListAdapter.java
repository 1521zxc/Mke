...
public class MealRightListAdapter extends TBaseAdapter<MealListBean.MealRight> {
    ...
    static int num=0;
    public MealRightListAdapter(Activity context, ArrayList<MealListBean.MealRight> list, ArrayList<RemarkBean> list2, double price, ArrayList<MealListBean.MealRight> lists, int num) {
        super(context, list);
        this.price=price;
        this.lists=lists;
        this.num=num;
    ...
    public void initItemView(final PxViewHolder view1, final ArrayList<MealListBean.MealRight> list, final int position) {
        ...
        mealRight.setLowSell(right.getLowSell());
        mealRight.setIconURL(right.getIconURL());
        mealRight.setChoose(right.getChoose());
        ...
        loadBitmap(mealRight.getIconURL(),view1.<ImageView>find(R.id.item_right_img), ImageView.ScaleType.CENTER_CROP);
        ...
        if(mealRight.getPrice()==0){
            ...
            view1.<TextView>find(R.id.item_right_price).setVisibility(View.VISIBLE);
            view1.<TextView>find(R.id.item_right_vip).setVisibility(View.VISIBLE);
        }
        ...
        if(right.getAttrs().size()>0)
        {
            ...
            view1.<LinearLayout>find(R.id.item_right_reduct).setVisibility(View.GONE);
        ...
        if(list.get(position).getSellStatus()==-1){
            //已售完
...
//
//        if(view1.<ImageView>find(R.id.item_right_add).getVisibility()==View.VISIBLE)
//        {
//            view1.<TextView>find(R.id.item_right_img1).setVisibility(View.GONE);
//        }
        if(mealRight.getChoose()==1){
            view1.<LinearLayout>find(R.id.item_right_reduct).setVisibility(View.INVISIBLE);
            view1.<TextView>find(R.id.item_right_num).setVisibility(View.INVISIBLE);
            view1.<LinearLayout>find(R.id.item_right_add).setVisibility(View.INVISIBLE);
            view1.<LinearLayout>find(R.id.item_right_gg).setVisibility(View.INVISIBLE);
        }
        ...
        view1.<LinearLayout>find(R.id.item_right_reduct).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(mealRight.getNumber()>0) {
                    ...
                    if(right.getNumber()==right.getLowSell()) {
                        mealRight.setNumber(0);
                        num=num-right.getLowSell();
                        price = price - (mealRight.getPrice()*right.getLowSell());
                    }else{
                        mealRight.setNumber(right.getNumber() - 1);
                        price = price - right.getPrice();
                        num--;
                    }
        ...
        view1.<LinearLayout>find(R.id.item_right_add).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(mealRight.getNumber()==0){
                    ...
                    mealRight.setNumber(mealRight.getNumber() +right.getLowSell());
                    num=num+right.getLowSell();
                    price=price+(mealRight.getPrice()*right.getLowSell());
                }else {
...
//               String remark=adapter2.getCh();
                ...
                if(lists.size()>0) {
                    ...
                    if(isE)
                    {
                        ...
                        mealRight.setChoose(mealRight.getChoose());
