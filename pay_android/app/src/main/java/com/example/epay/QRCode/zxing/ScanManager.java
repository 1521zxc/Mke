package com.example.epay.QRCode.zxing;
...
import com.example.epay.QRCode.zxing.camera.CameraManager;
import com.example.epay.QRCode.zxing.decode.DecodeThread;
import com.example.epay.QRCode.zxing.decode.PhotoScanHandler;
import com.example.epay.QRCode.zxing.decode.RGBLuminanceSource;
import com.example.epay.QRCode.zxing.utils.BeepManager;
import com.example.epay.QRCode.zxing.utils.BitmapUtil;
import com.example.epay.QRCode.zxing.utils.CaptureActivityHandler;
import com.example.epay.QRCode.zxing.utils.InactivityTimer;
    ...
    void initCamera(SurfaceHolder surfaceHolder) {
        ...
        try {
            ...
            if (handler == null) {
                ...
                Log.i("hongliang1", "handler new成功！:" + handler);
            }

            initCrop();
        } catch (IOException ioe) {
            Log.i(TAG, "hongliang", ioe);
            //弹出提示，报错
            ioe.printStackTrace();
            listener.scanError(new Exception("相机打开出错，请检查是否被禁止了该权限！"));
        } catch (RuntimeException e) {
            Log.i(TAG, "hongliang", e);
            //弹出提示，报错
            e.printStackTrace();
            listener.scanError(new Exception("相机打开出错，请检查是否被禁止了该权限！"));
        }
