...
public class OkHttpClientManager {
    ...
    private OkHttpClientManager() {
        ...
        mGson = new Gson();
        mOkHttpClient = new OkHttpClient().newBuilder().connectTimeout(30, TimeUnit.SECONDS) //链接超时
                ...
                .build();
        Log.e("dddddddddddddddd", "OkHttpClientManager: "+mOkHttpClient.connectTimeoutMillis());
        //cookie enabled
//        CookieStore

//        mOkHttpClient.setCookieHandler(new CookieManager(null, CookiePolicy.ACCEPT_ORIGINAL_SERVER));
//        mOkHttpClient.setCookieHandler(new CookieManager(
//               new PersistentCookieStore(BaseApplication.getInstance()), CookiePolicy.ACCEPT_ALL));
    ...
    public static Response getAsyn(String url, Object tag) throws IOException{
        return getInstance().getGetDelegate().getAsyn(url, tag);
    }
    ...
    public static void postSafe(String url, String command, Map<String, String> params) throws IOException {
        getInstance().getPostDelegate().postSafe(url, command, params);
    }

    public static Response postSafe(String url, String json) throws IOException {
        return getInstance().getPostDelegate().postSafe(url,json);
    }

    public static void postSafeAsyn(String url,String command,final ResultCallback callback,Map<String,String> params){
         getInstance().getPostDelegate().postSafeAsyn(url, command, callback, params);
    }

    public static void postSafeAsyn(String url,final ResultCallback callback,String json){
         getInstance().getPostDelegate().postSafeAsyn(url,callback,json);
    }
    ...
    public class PostDelegate {
         ...
         * @throws IOException
         */
        public Response postSafe(String url, String command, Map<String, String> params) throws IOException {
            Param[] paramsArr = map2Params(params);
            Request request = createPostRequest(url, command, paramsArr);
            Response response = mOkHttpClient.newCall(request).execute();
            return response;
        }

        public Response postSafe(String url, String json) throws IOException {
         ...
         * @param params
         */
        public void postSafeAsyn(String url,String command,final ResultCallback callback,Map<String,String> params){
            Param[] paramsArr = map2Params(params);
            Request request = createPostRequest(url,command,paramsArr);
            deliveryResult(callback, request);
        }
         ...
         * @param json
         */
        public void postSafeAsyn(String url ,final ResultCallback callback,String json){
            Request request = createPostRequest(url,json);
            deliveryResult(callback, request);
        }

        private Request createPostRequest(String url, String command, Param[] params) {
            RequestBody body = RequestBody.create(MEDIA_TYPE_JSON, encoding(command, params));
        ...
        private Request createPostRequest(String url , String json) {
            RequestBody body = RequestBody.create(MEDIA_TYPE_FORM, json);
            Request request = new Request.Builder()
                    .url(url)
                    .post(body)
                    .build();
        ...
        //封装请求body
        private String encoding(String command, Param[] params) {
            ...
            try {
                ...
                for (Param param : params) {
                    bodyJsonObject.put(param.key, param.value);
                }
            ...
            commandsArray.put(jsonObject);
            return commandsArray.toString();
        }

        //封装请求body
        private String encoding(String command, String json) {
            JSONObject jsonObject = new JSONObject();
            try {
                jsonObject.put("command", command);
                jsonObject.put("sequenceID", "");
                jsonObject.put("checkDigit", "");
                JSONObject bodyJsonObject = new JSONObject();
                jsonObject.put("body", JSON.parseObject(json));
            } catch (JSONException e) {
                e.printStackTrace();
            }
            JSONArray commandsArray = new JSONArray();
    ...
    public class GetDelegate {
        ...
        public Response getAsyn(String url, Object tag) throws IOException{
